...
const { collectGuideUiInventory } = require('./lib/plugin-guide-ui-inventory');
...
function withRuntimeLocalUrl(steps) {
  return steps.map((step) => ({
    ...step,
    copy: Object.fromEntries(LOCALES.map((locale) => [locale, {
      ...step.copy[locale],
      body: step.copy[locale].body.replaceAll('http://localhost:3000', '<current LTTH URL>')
    }]))
  }));
}
...
function buildGuides(repoRoot) {
  ...
  return records.map((record) => {
    ...
    const copy = manual ? manual.copy : localizedGuideCopy(name, entry);
    const overlay = manual && manual.overlay ? manual.overlay : entry.overlay;
    const steps = manual ? withRuntimeLocalUrl(manual.steps) : entry.steps;
    return {
      id: record.id,
      name,
      ...
      definition: createGuideDefinition({
        ...
        steps,
        overlay,
        inventory: collectGuideUiInventory(repoRoot, { definition: { activation: { route: entry.route } } })
      })
    };
