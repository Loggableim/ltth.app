...
async function saveConfig() {
    ...
    try {
        ...
        if (data.success) {
            ...
            if (animalCommandEditor) animalCommandEditor.load(config);
            const message = data.commandRegistration?.status === 'pending'
                ? pluginText(
                    'emoji_rain.commands_editor.registration_pending',
                    'Saved. Commands will be registered when GCCE is available.'
                )
                : pluginText('runtime.notifications.configuration_saved', 'Configuration saved.');
...
async function testHeartBalloons() {
    try {
        const response = await fetch('/api/emoji-rain/test-heart-balloons', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
...
async function testGiftBall() {
    try {
        const response = await fetch('/api/emoji-rain/test-gift-ball', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                giftName: 'Test Gift',
...
async function deleteImage(filename) {
    ...
    try {
        ...
        if (data.success) {
            ...
            const currentUrls = document.getElementById('image_urls').value.split('\n');
            const normalizedFilename = `/emoji-rain/uploads/${filename}`;
            const legacyFilename = `/uploads/emoji-rain/${filename}`;
