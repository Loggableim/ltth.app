...
describe('Interactive Story UI i18n', () => {
  ...
  test('requires the compact Story Studio translation contract', () => { const source = fs.readFileSync(path.join(repoRoot, 'app', 'plugins', pluginId, 'ui.html'), 'utf8'); for (const id of ['storyMode', 'dndJoinKeyword', 'dndInactivityLimitRounds', 'dndParticipantRoster', 'fishaudioModel', 'narrationEmotionMode', 'previewEditLayoutBtn', 'previewSaveLayoutBtn', 'previewResetLayoutBtn']) expect(source).toContain(`id="${id}"`); for (const locale of ['de', 'en', 'es', 'fr']) { const values = flattenTranslations(JSON.parse(fs.readFileSync(path.join(repoRoot, 'app', 'plugins', pluginId, 'locales', `${locale}.json`), 'utf8'))); for (const key of ['plugins.interactive-story.story_studio.title', 'plugins.interactive-story.story_studio.image_off', 'plugins.interactive-story.story_studio.fish_audio_default', 'plugins.interactive-story.story_studio.central_settings', 'plugins.interactive-story.story_studio.dnd.join_keyword', 'plugins.interactive-story.story_studio.players.empty', 'plugins.interactive-story.story_studio.preview.edit']) expect(values[key]).toEqual(expect.any(String)); } });
