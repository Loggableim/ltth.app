...
assert.strictEqual(shouldLoadPlugin('quiz-show', false, state1), false, 'Should use manifest enabled=false');
...
let state2 = {
    ...
    'quiz-show': { enabled: true }    // Override manifest enabled=false
};
assert.strictEqual(shouldLoadPlugin('webgpu-emoji-rain', true, state2), false, 'State should override manifest');
assert.strictEqual(shouldLoadPlugin('quiz-show', false, state2), true, 'State should override manifest');
...
assert.strictEqual(shouldLoadPlugin('quiz-show', false, state3), false, 'Should use manifest when no state');
...
const testState = {
    ...
    'quiz-show': { enabled: true, loadedAt: new Date().toISOString() }
};
...
assert.strictEqual(readState['quiz-show'].enabled, true, 'State file should contain quiz-show enabled');
