...
function bootUi() {
  ...
  const fetchMock = jest.fn(async (url, options = {}) => {
    ...
    if (url.startsWith('/api/streammonsters/gift-mappings/') && options.method === 'PUT') {
      return response({ success: true, mapping: JSON.parse(options.body) });
    }
...
describe('Stream Monsters creator wizard', () => {
  ...
  test('lets a creator map one gift to Random and sends Random to the API', async () => {
    const { dom, fetchMock } = bootUi();
    await waitFor(() => expect(dom.window.document.querySelector('#giftCatalog button')).not.toBeNull());

    const elementSelect = dom.window.document.getElementById('giftElement');
    expect(Array.from(elementSelect.options).map(option => option.value)).toContain('Random');
    dom.window.document.querySelector('#giftCatalog button').click();
    elementSelect.value = 'Random';
    ...
    const putCall = fetchMock.mock.calls.find(([url, options]) => (
      url === '/api/streammonsters/gift-mappings/5655' && options.method === 'PUT'
    ));
  ...
  test('saves multiple configurable aliases for a chat action', async () => {
    const { dom, fetchMock } = bootUi();
    await waitFor(() => expect(dom.window.document.getElementById('alias-eggs')).not.toBeNull());

    dom.window.document.getElementById('alias-eggs').value = 'eierliste, meineeier, eier';
    dom.window.document.getElementById('saveSetup').click();
    ...
    expect(JSON.parse(saveCalls.at(-1)[1].body)).toEqual(expect.objectContaining({
      commandAliases: expect.objectContaining({
        eggs: ['eierliste', 'meineeier', 'eier']
      })
    }));
    dom.window.close();
  });
