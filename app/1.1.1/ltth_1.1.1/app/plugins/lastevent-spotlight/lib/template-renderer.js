...
class TemplateRenderer {
  ...
  async render(userData, animate = true) {
    ...
    // Attach error handlers for images (CSP-compliant)
  ...
  /**
   * Attach error handlers to images for CSP-compliant fallback
   * This method must be called after DOM update
   */
  attachImageErrorHandlers() {
    if (!this.container) return;
    
    // Handle profile picture errors
    const profileImgs = this.container.querySelectorAll('.profile-img');
    ...
    // Handle gift icon errors
    const giftImgs = this.container.querySelectorAll('.gift-img');
  ...
  generateFallbackSvg(content, options = {}) {
    ...
    // Safely encode content, fallback to '?' if encoding fails
    let encodedContent;
    try {
      encodedContent = encodeURIComponent(content);
    } catch (error) {
      console.warn('Failed to encode content for SVG:', error);
      encodedContent = '?';
    }
   ...
   * Uses data attributes for CSP-compliant fallback handling
   */
  buildProfilePicture(userData, profilePicUrl, size, variant) {
    ...
    return `
      <div class="profile-picture" style="
        ...
        flex-shrink: 0;
      ">
        <img src="${escapedProfilePicUrl}" alt="${escapedNickname}" style="width: 100%; height: 100%; object-fit: cover;" data-fallback="${this.escapeHtml(fallbackSvg)}" class="profile-img" crossorigin="anonymous">
      </div>
    `;
  }
    ...
    if (giftPictureUrl && giftPictureUrl.trim() !== '') {
      ...
      return `
        <div class="gift-icon" style="
          ...
          flex-shrink: 0;
        ">
          <img src="${escapedGiftPictureUrl}" alt="${escapedGiftName}" style="width: 90%; height: 90%; object-fit: contain;" data-fallback="${this.escapeHtml(fallbackSvg)}" class="gift-img" crossorigin="anonymous">
        </div>
      `;
    } else {
