/**
 * Leaderboard Bar Overlay
 * Horizontal bar layout for top/bottom of stream
 */

class LeaderboardBar {
  constructor() {
    this.socket = null;
    this.data = [];
  ...
  parseUrlParams() {
    ...
    if (urlParams.has('showDelta')) {
      this.config.showDelta = urlParams.get('showDelta') !== 'false';
    }
  ...
  handleHypeStart() {
    document.querySelector('.bar-container').classList.add('hype-mode');
  }

  handleHypeEnd() {
    document.querySelector('.bar-container').classList.remove('hype-mode');
  }
  ...
  render() {
    ...
    if (!this.data || this.data.length === 0) {
      container.innerHTML = '<div class="empty-state">Waiting for gifts...</div>';
      return;
    }
    ...
    this.data.forEach((entry, index) => {
      ...
      html += `
        <div class="bar-entry ${animationClass}" data-user-id="${this.escapeHtml(userId)}">
...
document.addEventListener('DOMContentLoaded', () => {
  window.leaderboardBar = new LeaderboardBar();
});
