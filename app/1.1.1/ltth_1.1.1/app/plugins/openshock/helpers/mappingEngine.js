...
class MappingEngine {
  ...
  evaluateEvent(eventType, eventData) {
    try {
      ...
      this.logger.info(`[MappingEngine] 🔍 Evaluating ${eventType} event - ${candidates.length} enabled mappings found`);
      ...
      if (eventType === 'gift') {
        ...
        this.logger.info(`[MappingEngine]    Gift: ${giftName}, Coins: ${coins}`);
      }

      for (const mapping of candidates) {
        this.logger.debug(`[MappingEngine] 📋 Checking mapping "${mapping.name}" (ID: ${mapping.id})`);
        
        // Check conditions
        if (!this.checkConditions(mapping, eventData)) {
          this.logger.info(`[MappingEngine]    ❌ Conditions not met for mapping "${mapping.name}"`);
          continue;
        }
        ...
        if (!this.checkCooldown(mapping, userId, deviceId)) {
          this.logger.info(`[MappingEngine]    ⏳ Cooldown active for mapping "${mapping.name}"`);
          continue;
        }
        ...
        this.logger.info(`[MappingEngine]    ✅ Matched mapping "${mapping.name}"`);
      }
      ...
      if (eventType === 'gift' && matches.length > 0) {
        ...
        if (hasSpecificGiftMapping) {
          ...
          this.logger.info(`[MappingEngine] 🎯 Specific gift mapping found, filtering ${matches.length - filteredMatches.length} generic mappings`);
          matches.length = 0;
      ...
      this.logger.info(`[MappingEngine] 🎉 Evaluation complete - ${matches.length} mapping(s) matched`);
      
      return matches;
    } catch (error) {
      this.logger.error('[MappingEngine] ❌ Error evaluating event:', error.message);
      this.logger.error(`[MappingEngine]    Stack: ${error.stack}`);
      return [];
    }
  }
  ...
  checkConditions(mapping, eventData) {
    ...
    const mappingName = mapping.name || mapping.id;
    ...
    try {
      ...
      if (conditions.whitelist && conditions.whitelist.length > 0) {
        if (!conditions.whitelist.includes(userId) && !conditions.whitelist.includes(userName)) {
          this.logger.debug(`[MappingEngine] ❌ "${mappingName}": User not in whitelist (userId: ${userId}, userName: ${userName})`);
          return false;
        }
        this.logger.debug(`[MappingEngine] ✅ "${mappingName}": User in whitelist`);
      }
      ...
      if (conditions.blacklist && conditions.blacklist.length > 0) {
        if (conditions.blacklist.includes(userId) || conditions.blacklist.includes(userName)) {
          this.logger.debug(`[MappingEngine] ❌ "${mappingName}": User in blacklist (userId: ${userId}, userName: ${userName})`);
          return false;
        }
        this.logger.debug(`[MappingEngine] ✅ "${mappingName}": User not in blacklist`);
      }

      // Team level check
      if (conditions.teamLevelMin !== undefined && conditions.teamLevelMin > 0) {
        const teamLevel = eventData.user?.teamLevel || eventData.teamMemberLevel || 0;
        if (teamLevel < conditions.teamLevelMin) {
          this.logger.debug(`[MappingEngine] ❌ "${mappingName}": Team level ${teamLevel} < required ${conditions.teamLevelMin}`);
          return false;
        }
        this.logger.debug(`[MappingEngine] ✅ "${mappingName}": Team level ${teamLevel} >= required ${conditions.teamLevelMin}`);
      }
      ...
      if (conditions.followerAgeMin !== undefined && conditions.followerAgeMin > 0) {
        ...
        if (followTime) {
          ...
          if (ageInDays < conditions.followerAgeMin) {
            this.logger.debug(`[MappingEngine] ❌ "${mappingName}": Follower age ${ageInDays.toFixed(1)} days < required ${conditions.followerAgeMin} days`);
            return false;
          }
          this.logger.debug(`[MappingEngine] ✅ "${mappingName}": Follower age ${ageInDays.toFixed(1)} days >= required ${conditions.followerAgeMin} days`);
        }
      ...
      switch (mapping.eventType) {
        case 'gift':
          ...
          if (conditions.giftName) {
            ...
            this.logger.debug(`[MappingEngine] 🎁 "${mappingName}": Comparing gift names - Expected: "${conditions.giftName}", Received: "${giftName}"`);
            
            // Support wildcard matching with '*' or empty string to match any gift
            // Handle null/undefined giftName safely
            const isWildcard = !conditions.giftName || 
                              conditions.giftName === '*' || 
                              (typeof conditions.giftName === 'string' && conditions.giftName.trim() === '');
            
            if (!isWildcard) {
              // Case-insensitive comparison for specific gift names
              if (giftName.toLowerCase() !== conditions.giftName.toLowerCase()) {
                this.logger.debug(`[MappingEngine] ❌ "${mappingName}": Gift name mismatch - "${giftName}" !== "${conditions.giftName}"`);
                return false;
              }
              
              this.logger.debug(`[MappingEngine] ✅ "${mappingName}": Gift name matched`);
            } else {
              this.logger.debug(`[MappingEngine] ✅ "${mappingName}": Wildcard - accepting any gift`);
            }
          } else {
            this.logger.debug(`[MappingEngine] ✅ "${mappingName}": No gift name filter - accepting any gift`);
          }
          
          // Check coin range if specified
          const coins = eventData.giftCoins || eventData.coins || 0;
          
          this.logger.debug(`[MappingEngine] 💰 "${mappingName}": Gift coins: ${coins}, minCoins: ${conditions.minCoins || 'none'}, maxCoins: ${conditions.maxCoins || 'none'}`);
          
          if (conditions.minCoins !== undefined && coins < conditions.minCoins) {
            this.logger.debug(`[MappingEngine] ❌ "${mappingName}": Coins ${coins} below minimum ${conditions.minCoins}`);
            return false;
          }
          
          if (conditions.maxCoins !== undefined && coins > conditions.maxCoins) {
            this.logger.debug(`[MappingEngine] ❌ "${mappingName}": Coins ${coins} above maximum ${conditions.maxCoins}`);
            return false;
          }
          
          if (conditions.minCoins !== undefined || conditions.maxCoins !== undefined) {
            this.logger.debug(`[MappingEngine] ✅ "${mappingName}": Coin range check passed`);
          }
