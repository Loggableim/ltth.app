...
class StreamAlchemyPlugin {
  ...
  async init() {
    ...
    this.streamMonstersViewerActivity = new StreamMonstersViewerActivityTracker({
      activeWindowMs: this.config.streamMonsters.autoHatchActiveWindowSeconds
        * 1_000
    });
  ...
  markStreamMonstersViewerActive(userId) {
    const streamKey = this.streamMonstersEngine?.streamKey;
    if (!userId || !streamKey || streamKey === 'offline') return false;
    return Boolean(this.streamMonstersViewerActivity?.observe?.({ userId, streamKey }));
  }
  ...
  buildStreamMonstersCommandDefinitions(commandPrefix = this.streamMonstersCommandPrefix) {
    ...
    return [
      ...
      handler: async (args, context = {}) => {
        ...
        this.markStreamMonstersViewerActive(userId);
