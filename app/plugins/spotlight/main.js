 ...
 * Spotlight Plugin
 *
 * Provides nine permanent live overlays for recent follower, like, chat, share,
 * gift, subscriber, top gift, gift streak, and rotation spotlight views.
 * Supports real-time updates via WebSocket and comprehensive customization settings.
 */

class SpotlightPlugin {
  constructor(api) {
    this.api = api;
    this.pluginId = 'spotlight';
   ...
   */
  async init() {
    this.api.log('Spotlight plugin loading...');
    ...
    this.api.log('Spotlight plugin loaded successfully');
  }
  ...
  registerRoutes() {
    ...
    const registerRouteAlias = (method, primaryPath, handler, legacyPath = null) => {
      this.api.registerRoute(method, primaryPath, handler);
      if (legacyPath) {
        this.api.registerRoute(method, legacyPath, handler);
      }
    };

    // Serve overlay HTML files
    for (const type of Object.keys(this.eventTypes)) {
      registerRouteAlias('GET', `/overlay/spotlight/${type}`, (req, res) => {
        const overlayPath = path.join(__dirname, 'overlays', `${type}.html`);
        res.sendFile(overlayPath);
      }, `/overlay/lastevent/${type}`);
      
      // Serve overlay JS files
      registerRouteAlias('GET', `/plugins/spotlight/overlays/${type}.js`, (req, res) => {
        const jsPath = path.join(__dirname, 'overlays', `${type}.js`);
        res.sendFile(jsPath);
      }, `/plugins/lastevent-spotlight/overlays/${type}.js`);
    }

    // Serve plugin UI
    registerRouteAlias('GET', '/spotlight/ui', (req, res) => {
      const uiPath = path.join(__dirname, 'ui', 'main.html');
      res.sendFile(uiPath);
    }, '/lastevent-spotlight/ui');
    
    // Serve UI JS
    registerRouteAlias('GET', '/plugins/spotlight/ui/main.js', (req, res) => {
      const jsPath = path.join(__dirname, 'ui', 'main.js');
      res.sendFile(jsPath);
    }, '/plugins/lastevent-spotlight/ui/main.js');

    // Serve library files
    registerRouteAlias('GET', '/plugins/spotlight/lib/animations.js', (req, res) => {
      const libPath = path.join(__dirname, 'lib', 'animations.js');
      res.sendFile(libPath);
    }, '/plugins/lastevent-spotlight/lib/animations.js');

    registerRouteAlias('GET', '/plugins/spotlight/lib/text-effects.js', (req, res) => {
      const libPath = path.join(__dirname, 'lib', 'text-effects.js');
      res.sendFile(libPath);
    }, '/plugins/lastevent-spotlight/lib/text-effects.js');

    registerRouteAlias('GET', '/plugins/spotlight/lib/template-renderer.js', (req, res) => {
      const libPath = path.join(__dirname, 'lib', 'template-renderer.js');
      res.sendFile(libPath);
    }, '/plugins/lastevent-spotlight/lib/template-renderer.js');

    registerRouteAlias('GET', '/plugins/spotlight/overlays/single-overlay.js', (req, res) => {
      const jsPath = path.join(__dirname, 'overlays', 'single-overlay.js');
      res.sendFile(jsPath);
    }, '/plugins/lastevent-spotlight/overlays/single-overlay.js');
   ...
   */
  async destroy() {
    this.api.log('Spotlight plugin unloading...');
    this.cancelAllPendingPersists();
  }
}
