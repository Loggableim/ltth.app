...
function readSettingRecord(db, key) {
    const row = db.prepare('SELECT value FROM settings WHERE key = ?').get(key);
    if (!row) {
        return { exists: false, value: null, valid: false };
    }
    try {
        return { exists: true, value: JSON.parse(row.value), valid: true };
    } catch (error) {
        return { exists: true, value: null, valid: false };
    }
}
...
function migrateWebgpuWeatherStorage(databaseManager, generateApiKey) {
    ...
    const existingConfig = readSettingRecord(db, CONFIG_KEY);
    const existingMarker = readSettingRecord(db, BOOTSTRAP_KEY);
    // Existence, not JSON validity, is the conservative ownership boundary.
    if (existingConfig.exists || existingMarker.exists) {
        return {
            migrated: false,
            config: existingConfig.valid ? existingConfig.value : null,
            diagnostic: existingConfig.exists && !existingConfig.valid
                ? 'existing-webgpu-config-is-invalid'
                : existingMarker.exists && !existingMarker.valid
                    ? 'existing-webgpu-bootstrap-marker-is-invalid'
                    : 'existing-webgpu-storage-preserved'
        };
    }

    const classicConfig = readSettingRecord(db, CLASSIC_CONFIG_KEY);
    const config = createInitialWebgpuWeatherConfig(classicConfig.valid ? classicConfig.value : null, generateApiKey);
    Object.values(config.effects).forEach((effect) => { effect.permanent = false; });
    config.enabled = false;
    config.gamification.enabled = false;
    config.gamification.overlay.enabled = false;
    const mappings = db.prepare("SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'gift_weather_mappings'").get()
        ? db.prepare('SELECT gift_id, weather_effect, intensity, duration, enabled FROM gift_weather_mappings').all()
        : [];
    ...
    db.transaction(() => {
        ...
        const insert = db.prepare(`
            ...
            VALUES (?, ?, ?, ?, ?)
        `);
        mappings.forEach((mapping) => insert.run(
            mapping.gift_id, mapping.weather_effect, mapping.intensity, mapping.duration, mapping.enabled
        ));
        writeSetting(db, BOOTSTRAP_KEY, marker);
    })();

    return { migrated: true, config, diagnostic: 'webgpu-bootstrap-complete' };
}
 ...
 *
 * Features:
 * - Renderer-ready WebGPU control plane (renderer supplied separately)
...
class WebgpuWeatherControlPlugin {
    ...
    validateQualityPreset(value) {
        const allowed = ['auto', 'low', 'medium', 'high', 'ultra'];
    ...
    registerWebgpuRoute(method, routePath, handler) {
     ...
     */
    getGCCEInstance() {
        return this.api.getPlugin?.('gcce') || this.api.pluginLoader?.loadedPlugins?.get('gcce')?.instance || null;
    }

    async init() {
        this.api.log('ÃƒÂ°Ã…Â¸Ã…â€™Ã‚Â¦ÃƒÂ¯Ã‚Â¸Ã‚Â [WEATHER CONTROL] Initializing Weather Control Plugin...', 'info');

        // Load configuration
        await this.loadConfig();

        // Register routes
        this.api.log('ÃƒÂ°Ã…Â¸Ã¢â‚¬ÂºÃ‚Â£ÃƒÂ¯Ã‚Â¸Ã‚Â [WEATHER CONTROL] Registering routes...', 'debug');
        this.registerRoutes();

        // Register TikTok event handlers
        this.api.log('ÃƒÂ°Ã…Â¸Ã…Â½Ã‚Â¯ [WEATHER CONTROL] Registering TikTok event handlers...', 'debug');
        this.registerTikTokEventHandlers();

        // Register flow actions
        this.api.log('ÃƒÂ¢Ã…Â¡Ã‚Â¡ [WEATHER CONTROL] Registering flow actions...', 'debug');
        this.registerFlowActions();

        // Register GCCE commands
        this.api.log('ÃƒÂ°Ã…Â¸Ã¢â‚¬â„¢Ã‚Â¬ [WEATHER CONTROL] Registering GCCE commands...', 'debug');
        ...
        this.api.log('ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ [WEATHER CONTROL] Weather Control Plugin initialized successfully', 'info');
    }

    async loadConfig() {
        try {
            const migration = migrateWebgpuWeatherStorage(this.api.getDatabase(), () => this.generateApiKey());
            const config = migration.config;
            this.storageDiagnostic = migration.diagnostic;
            this.storageBlocked = migration.diagnostic.startsWith('existing-webgpu-') && !config;
            ...
            // A malformed pre-existing WebGPU row is intentionally never repaired
            // implicitly. The UI receives a diagnostic and can guide recovery.
            if (!this.storageBlocked) {
                await this.persistConfig();
            }

            this.api.log('ÃƒÂ°Ã…Â¸Ã¢â‚¬Å“Ã‚Â [WEATHER CONTROL] Configuration loaded', 'debug');
        } catch (error) {
            this.api.log(`ÃƒÂ¢Ã‚ÂÃ…â€™ [WEATHER CONTROL] Error loading config: ${error.message}`, 'error');
            throw error;
        }
    }
    ...
    registerRoutes() {
        ...
        this.api.registerRoute('get', '/webgpu-weather-control/ui', (req, res) => {
            const uiPath = path.join(__dirname, 'ui.html');
            res.sendFile(uiPath);
        });

        // Serve plugin overlay
        this.api.registerRoute('get', '/webgpu-weather-control/overlay', (req, res) => {
        ...
        this.registerAdminRoute('get', '/api/webgpu-weather/config', async (req, res) => {
            try {
                ...
                res.json({ success: true, config: safeConfig, diagnostic: this.storageDiagnostic });
            } catch (error) {
                this.api.log(`ÃƒÂ¢Ã‚ÂÃ…â€™ [WEATHER CONTROL] Error getting config: ${error.message}`, 'error');
                res.status(500).json({ success: false, error: error.message });
            }
        });

        // Update configuration
        this.registerWebgpuRoute('post', '/api/webgpu-weather/config', async (req, res) => {
            try {
                ...
                if (wasEnabled && !this.config.enabled) {
                    this.api.emit('webgpu-weather:stop', {
                        username: 'system',
                ...
                if (effectsChanged || wasEnabled !== this.config.enabled) {
                    this.api.log('ÃƒÂ¢Ã¢â€žÂ¢Ã‚Â¾ÃƒÂ¯Ã‚Â¸Ã‚Â [WEATHER CONTROL] Permanent effects changed, syncing...', 'info');
                }
                this.syncPermanentEffects();

                // Every runtime-facing setting change must reach already-open OBS overlays.
                this.api.emit('webgpu-weather:config-changed', {
                    timestamp: Date.now(),
                ...
                if (commandNamesChanged) {
                    this.api.log('ÃƒÂ°Ã…Â¸Ã¢â‚¬â„¢Ã‚Â¬ [WEATHER CONTROL] Command names changed, re-registering...', 'info');
                    this.registerGCCECommands();
                }

                this.api.log('ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ [WEATHER CONTROL] Configuration updated', 'info');
                res.json({ success: true, config: this.config });
            } catch (error) {
                this.api.log(`ÃƒÂ¢Ã‚ÂÃ…â€™ [WEATHER CONTROL] Error updating config: ${error.message}`, 'error');
                res.status(500).json({ success: false, error: error.message });
            }
        });

        // Main weather trigger endpoint
        this.registerWebgpuRoute('post', '/api/webgpu-weather/trigger', async (req, res) => {
            try {
                if (!this.config.enabled) {
                ...
                if (!this.config.useGlobalAuth) {
                    const providedKey = req.headers['x-webgpu-weather-key'];
                    if (!this.apiKeysEqual(providedKey, this.apiKey)) {
                        this.api.log('ÃƒÂ°Ã…Â¸Ã…Â¡Ã‚Â« [WEATHER CONTROL] Invalid API key attempt', 'warn');
                ...
                if (!rateLimitResult.allowed) {
                    this.api.log(`ÃƒÂ¢Ã‚ÂÃ‚Â±ÃƒÂ¯Ã‚Â¸Ã‚Â [WEATHER CONTROL] Rate limit exceeded for ${username || req.ip}`, 'warn');
                ...
                if (username && this.config.permissions.enabled) {
                    ...
                    if (!hasPermission) {
                        this.api.log(`ÃƒÂ°Ã…Â¸Ã…Â¡Ã‚Â« [WEATHER CONTROL] Permission denied for user ${username}`, 'warn');

                        // Optional: Send feedback to overlay/chat
                        this.api.emit('webgpu-weather:permission-denied', {
                            username,
                            action,
                            timestamp: Date.now()
                        });
                ...
                // Log event
                this.api.log(`ÃƒÂ°Ã…Â¸Ã…â€™Ã‚Â¦ÃƒÂ¯Ã‚Â¸Ã‚Â [WEATHER CONTROL] Triggered: ${action} (intensity: ${validIntensity}, duration: ${validDuration}ms) by ${username || 'API'}`, 'info');

                // Emit to all overlay clients via WebSocket
                this.api.emit('webgpu-weather:trigger', weatherEvent);
        ...
        this.registerWebgpuRoute('post', '/api/webgpu-weather/stop', async (req, res) => {
            try {
                ...
                if (action) {
                    this.api.emit('webgpu-weather:stop-effect', {
                        action,
                ...
                this.api.log(`[WEATHER CONTROL] Error stopping webgpu-weather: ${error.message}`, 'error');
                res.status(500).json({ success: false, error: error.message });
            }
        });

        // Trigger a timed weather sequence
        this.registerWebgpuRoute('post', '/api/webgpu-weather/sequence/trigger', async (req, res) => {
            try {
                if (!this.config.enabled) {
                    return res.status(403).json({ success: false, error: 'Weather Control is disabled' });
                }
        ...
        this.api.registerRoute('get', '/api/webgpu-weather/effects', (req, res) => {
            res.json({
                success: true,
        ...
        this.api.registerRoute('get', '/api/webgpu-weather/gamification', (req, res) => {
            try {
                res.json({
                    success: true,
                ...
                this.api.log(`ÃƒÂ¢Ã‚ÂÃ…â€™ [WEATHER CONTROL] Error getting gamification state: ${error.message}`, 'error');
                res.status(500).json({ success: false, error: error.message });
            }
        });

        // Reset gamification progress
        this.registerWebgpuRoute('post', '/api/webgpu-weather/gamification/reset', async (req, res) => {
            try {
                ...
                this.api.log(`ÃƒÂ¢Ã‚ÂÃ…â€™ [WEATHER CONTROL] Error resetting gamification: ${error.message}`, 'error');
                res.status(500).json({ success: false, error: error.message });
            }
        });
        ...
        this.registerAdminRoute('post', '/api/webgpu-weather/reset-key', async (req, res) => {
            try {
                ...
                this.api.log('ÃƒÂ°Ã…Â¸Ã¢â‚¬ÂÃ¢â‚¬Ëœ [WEATHER CONTROL] API key reset', 'info');
                res.json({ success: true, apiKey: this.apiKey });
            } catch (error) {
                this.api.log(`ÃƒÂ¢Ã‚ÂÃ…â€™ [WEATHER CONTROL] Error resetting API key: ${error.message}`, 'error');
                res.status(500).json({ success: false, error: error.message });
            }
        });

        // Get all gift-to-weather mappings
        this.api.registerRoute('get', '/api/webgpu-weather/gift-mappings', (req, res) => {
            try {
                const db = this.api.getDatabase();
                const mappings = db.getAllWebgpuGiftWeatherMappings();
                res.json({ success: true, mappings });
            } catch (error) {
                this.api.log(`ÃƒÂ¢Ã‚ÂÃ…â€™ [WEATHER CONTROL] Error getting gift mappings: ${error.message}`, 'error');
                res.status(500).json({ success: false, error: error.message });
            }
        });

        // Get specific gift-to-weather mapping
        this.api.registerRoute('get', '/api/webgpu-weather/gift-mappings/:giftId', (req, res) => {
            try {
                const db = this.api.getDatabase();
                const mapping = db.getWebgpuGiftWeatherMapping(req.params.giftId);
                res.json({ success: true, mapping });
            } catch (error) {
                this.api.log(`ÃƒÂ¢Ã‚ÂÃ…â€™ [WEATHER CONTROL] Error getting gift mapping: ${error.message}`, 'error');
                res.status(500).json({ success: false, error: error.message });
            }
        });

        // Create or update gift-to-weather mapping
        this.registerWebgpuRoute('post', '/api/webgpu-weather/gift-mappings', (req, res) => {
            try {
                ...
                const db = this.api.getDatabase();
                db.setWebgpuGiftWeatherMapping(
                    giftId,
                    weatherEffect,
                    validIntensity,
                    validDuration,
                ...
                this.api.log(`ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ [WEATHER CONTROL] Gift mapping created/updated: Gift ${giftId} -> ${weatherEffect}`, 'info');
                res.json({ success: true });
            } catch (error) {
                this.api.log(`ÃƒÂ¢Ã‚ÂÃ…â€™ [WEATHER CONTROL] Error creating/updating gift mapping: ${error.message}`, 'error');
                res.status(500).json({ success: false, error: error.message });
            }
        });

        // Delete gift-to-weather mapping
        this.registerWebgpuRoute('delete', '/api/webgpu-weather/gift-mappings/:giftId', (req, res) => {
            try {
                const db = this.api.getDatabase();
                db.deleteWebgpuGiftWeatherMapping(req.params.giftId);

                this.api.log(`ÃƒÂ°Ã…Â¸Ã¢â‚¬â€Ã¢â‚¬ËœÃƒÂ¯Ã‚Â¸Ã‚Â [WEATHER CONTROL] Gift mapping deleted: Gift ${req.params.giftId}`, 'info');
                res.json({ success: true });
            } catch (error) {
                this.api.log(`ÃƒÂ¢Ã‚ÂÃ…â€™ [WEATHER CONTROL] Error deleting gift mapping: ${error.message}`, 'error');
                res.status(500).json({ success: false, error: error.message });
            }
        });
    }
    ...
    registerTikTokEventHandlers() {
        ...
        this.api.registerTikTokEvent('gift', async (data) => {
            try {
                ...
                const giftMapping = db.getWebgpuGiftWeatherMapping(giftId);
                ...
                if (giftMapping && giftMapping.enabled) {
                    ...
                    this.api.log(`ÃƒÂ°Ã…Â¸Ã…Â½Ã‚Â [WEATHER CONTROL] Using gift mapping for gift ${giftId}: ${weatherAction}`, 'debug');
                } else {
                ...
                if (weatherAction && this.config.effects[weatherAction]?.enabled) {
                    ...
                    if (this.config.permissions.enabled) {
                        const hasPermission = await this.checkUserPermission(username);
                        if (!hasPermission) {
                            this.api.log(`ÃƒÂ°Ã…Â¸Ã…Â¡Ã‚Â« [WEATHER CONTROL] Permission denied for gift from ${username}`, 'debug');
                            return;
                        }
                    }
                    ...
                    if (!rateLimitResult.allowed) {
                        this.api.log(`ÃƒÂ¢Ã‚ÂÃ‚Â±ÃƒÂ¯Ã‚Â¸Ã‚Â [WEATHER CONTROL] Rate limit exceeded for ${username}`, 'debug');
                        return;
                    }
                    ...
                    this.api.log(`ÃƒÂ°Ã…Â¸Ã…Â½Ã‚Â [WEATHER CONTROL] Gift triggered: ${weatherAction} by ${username} (${giftName})`, 'info');
                    this.api.emit('webgpu-weather:trigger', weatherEvent);
                }
            } catch (error) {
                this.api.log(`ÃƒÂ¢Ã‚ÂÃ…â€™ [WEATHER CONTROL] Error in gift handler: ${error.message}`, 'error');
            }
        });
    ...
    registerFlowActions() {
        ...
        this.api.registerFlowAction('webgpu-weather.trigger', async (params) => {
            try {
                ...
                this.api.log(`ÃƒÂ¢Ã…Â¡Ã‚Â¡ [WEATHER CONTROL] Flow triggered: ${action}`, 'info');
                this.api.emit('webgpu-weather:trigger', weatherEvent);

                return { success: true, event: weatherEvent };
            } catch (error) {
                this.api.log(`ÃƒÂ¢Ã‚ÂÃ…â€™ [WEATHER CONTROL] Error in flow action: ${error.message}`, 'error');
    ...
    registerSocketEndpoints() {
        this.api.registerSocket('webgpu-weather:trigger', async (socket, params = {}) => {
            if (!this.apiKeysEqual(params.key, this.apiKey)) return;
            const event = this.createWeatherEvent({ ...params, username: params.username || 'socket' });
            if (this.config.enabled && this.supportedEffects.includes(params.action) && this.config.effects[params.action]?.enabled) {
                this.emitWeatherEvent(event);
            }
        });
        this.api.registerSocket('webgpu-weather:stop', async (socket, params = {}) => {
            if (!this.apiKeysEqual(params.key, this.apiKey)) return;
            const action = this.supportedEffects.includes(params.action) ? params.action : null;
            this.api.emit(action ? 'webgpu-weather:stop-effect' : 'webgpu-weather:stop', { action, timestamp: Date.now() });
        });
    }
        ...
        try {
            ...
            this.socketConnectionHandler = (socket) => {
                try {
                    ...
                    socket.on('webgpu-weather:client-ready', handlers.clientReady);
                    ...
                    this.api.log(`ÃƒÂ¢Ã‚ÂÃ…â€™ [WEATHER CONTROL] Error syncing permanent effects: ${error.message}`, 'error');
                }
            };
            ...
            this.api.log(`ÃƒÂ¢Ã‚ÂÃ…â€™ [WEATHER CONTROL] Error registering socket sync: ${error.message}`, 'error');
        }
    }
    ...
    syncPermanentEffects(targetSocket = null) {
        ...
        if (targetSocket) {
            ...
            // ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ FIX: Update activePermanentEffects during socket sync
            ...
            this.api.log(`ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦ [WEATHER CONTROL] Synced ${desiredEffects.size} permanent effects to new client`, 'debug');
            return;
        }
        ...
        this.activePermanentEffects.forEach((effect) => {
            if (!desiredEffects.has(effect)) {
                this.api.emit('webgpu-weather:stop-effect', { action: effect, meta: { triggeredBy: 'permanent-toggle' } });
                this.api.log(`ÃƒÂ°Ã…Â¸Ã¢â‚¬ÂºÃ¢â‚¬Ëœ [WEATHER CONTROL] Stopped permanent effect: ${effect}`, 'info');
            }
        });

        desiredEffects.forEach((effect) => {
            this.emitPermanentEffect(effect);
            this.api.log(`ÃƒÂ¢Ã¢â€žÂ¢Ã‚Â¾ÃƒÂ¯Ã‚Â¸Ã‚Â [WEATHER CONTROL] Synced permanent effect: ${effect}`, 'info');
        });
    ...
    emitPermanentEffect(effect, socket = null) {
        ...
        if (socket) {
            socket.emit('webgpu-weather:trigger', payload);
        } else {
            this.api.emit('webgpu-weather:trigger', payload);
        }
    }
    ...
    registerGCCECommands() {
        try {
            ...
            if (!gcce) {
                this.api.log('ÃƒÂ°Ã…Â¸Ã¢â‚¬â„¢Ã‚Â¬ [WEATHER CONTROL] GCCE not available, skipping command registration', 'debug');
                return;
            }

            if (!this.config.chatCommands.enabled) {
                this.api.log('ÃƒÂ°Ã…Â¸Ã¢â‚¬â„¢Ã‚Â¬ [WEATHER CONTROL] Chat commands disabled in config', 'debug');
                return;
            }
            ...
            try {
                gcce.unregisterCommandsForPlugin('webgpu-weather-control');
            } catch (e) {
                // Ignore errors if commands don't exist yet
            }

            // Register commands with GCCE
            const result = gcce.registerCommandsForPlugin('webgpu-weather-control', commands);

            this.api.log(`ÃƒÂ°Ã…Â¸Ã¢â‚¬â„¢Ã‚Â¬ [WEATHER CONTROL] Registered ${result.registered.length} commands with GCCE`, 'info');

            if (result.failed.length > 0) {
                this.api.log(`ÃƒÂ°Ã…Â¸Ã¢â‚¬â„¢Ã‚Â¬ [WEATHER CONTROL] Failed to register commands: ${result.failed.join(', ')}`, 'warn');
            }

        } catch (error) {
            this.api.log(`ÃƒÂ¢Ã‚ÂÃ…â€™ [WEATHER CONTROL] Error registering GCCE commands: ${error.message}`, 'error');
        }
    }
    ...
    async handleWeatherCommand(args, context) {
        try {
            ...
            if (this.config.chatCommands.requirePermission && this.config.permissions.enabled) {
                ...
                if (!hasPermission) {
                    this.api.log(`ÃƒÂ°Ã…Â¸Ã…Â¡Ã‚Â« [WEATHER CONTROL] Permission denied for user ${context.username}`, 'debug');
            ...
            if (!rateLimitResult.allowed) {
                this.api.log(`ÃƒÂ¢Ã‚ÂÃ‚Â±ÃƒÂ¯Ã‚Â¸Ã‚Â [WEATHER CONTROL] Rate limit exceeded for ${context.username}`, 'debug');
                return {
                    success: false,
            ...
            // Log and emit
            this.api.log(`ÃƒÂ°Ã…Â¸Ã…â€™Ã‚Â¦ÃƒÂ¯Ã‚Â¸Ã‚Â [WEATHER CONTROL] Chat command triggered: ${effectName} by ${context.username}`, 'info');
            this.api.emit('webgpu-weather:trigger', weatherEvent);
            ...
            this.api.log(`ÃƒÂ¢Ã‚ÂÃ…â€™ [WEATHER CONTROL] Error in weather command: ${error.message}`, 'error');
            return {
                success: false,
    ...
    async handleWeatherListCommand(args, context) {
        try {
            ...
            const effectEmojis = {
                rain: 'ÃƒÂ°Ã…Â¸Ã…â€™Ã‚Â§ÃƒÂ¯Ã‚Â¸Ã‚Â',
                snow: 'ÃƒÂ¢Ã‚ÂÃ¢â‚¬Å¾ÃƒÂ¯Ã‚Â¸Ã‚Â',
                storm: 'ÃƒÂ¢Ã¢â‚¬ÂºÃ‹â€ ÃƒÂ¯Ã‚Â¸Ã‚Â',
                fog: 'ÃƒÂ°Ã…Â¸Ã…â€™Ã‚Â«ÃƒÂ¯Ã‚Â¸Ã‚Â',
                thunder: 'ÃƒÂ¢Ã…Â¡Ã‚Â¡',
                sunbeam: 'ÃƒÂ¢Ã‹Å“Ã¢â€šÂ¬ÃƒÂ¯Ã‚Â¸Ã‚Â',
                glitchclouds: 'ÃƒÂ¢Ã‹Å“Ã‚ÂÃƒÂ¯Ã‚Â¸Ã‚Â'
            };

            const effectList = enabledEffects.map(effect =>
                `${effectEmojis[effect] || 'ÃƒÂ°Ã…Â¸Ã…â€™Ã‚Â¦ÃƒÂ¯Ã‚Â¸Ã‚Â'} ${effect}`
            ).join(', ');
            ...
            this.api.log(`ÃƒÂ¢Ã‚ÂÃ…â€™ [WEATHER CONTROL] Error in weatherlist command: ${error.message}`, 'error');
            return {
                success: false,
    ...
    async handleWeatherStopCommand(args, context) {
        try {
            ...
            this.api.log(`ÃƒÂ°Ã…Â¸Ã¢â‚¬ÂºÃ¢â‚¬Ëœ [WEATHER CONTROL] Weather effects stopped by ${context.username}`, 'info');
            ...
            this.api.log(`ÃƒÂ¢Ã‚ÂÃ…â€™ [WEATHER CONTROL] Error in weatherstop command: ${error.message}`, 'error');
            return {
                success: false,
    ...
    async checkUserPermission(username, contextUserData = null) {
        try {
            ...
            if (contextUserData?.dbUser) {
                ...
                this.api.log('ÃƒÂ°Ã…Â¸Ã¢â‚¬ÂÃ‚Â [WEATHER CONTROL] Using cached user data from GCCE', 'debug');
            } else {
                // Fallback: Get user data from database
                const db = this.api.getDatabase();
                user = db.prepare('SELECT * FROM users WHERE username = ?').get(username);
                this.api.log('ÃƒÂ°Ã…Â¸Ã¢â‚¬ÂÃ‚Â [WEATHER CONTROL] Fetching user data from database (fallback)', 'debug');
            }
            ...
            this.api.log(`ÃƒÂ¢Ã‚ÂÃ…â€™ [WEATHER CONTROL] Error checking permissions: ${error.message}`, 'error');
    ...
    scheduleGamificationPersist() {
        ...
        this.gamificationPersistTimer = setTimeout(() => {
            this.persistGamificationState().catch((error) => {
                this.api.log(`ÃƒÂ¢Ã‚ÂÃ…â€™ [WEATHER CONTROL] Error persisting gamification state: ${error.message}`, 'error');
            });
        }, 150);
    }
    ...
    broadcastGamificationState(reason = 'update', extra = {}) {
        ...
        this.api.emit('webgpu-weather:gamification-state', payload);
    ...
    advanceQuestProgress(eventType, payload = {}, contribution = null) {
        ...
        if (quest.progress >= quest.target) {
            ...
            this.api.emit('webgpu-weather:gamification-quest', {
                reason: 'completed',
                quest: { ...quest }
            });
    ...
    resolveRewardThreshold(snapshot = null) {
        ...
        for (const threshold of thresholds) {
            ...
            if (meter >= threshold.meter && !onCooldown && !alreadyFired) {
                ...
                this.api.emit('webgpu-weather:trigger', rewardEvent);
     ...
     */
    async destroy() {
        this.api.log('ÃƒÂ°Ã…Â¸Ã…â€™Ã‚Â¦ÃƒÂ¯Ã‚Â¸Ã‚Â [WEATHER CONTROL] Destroying Weather Control Plugin...', 'info');
        ...
        try {
            ...
            if (gcce) {
                gcce.unregisterCommandsForPlugin('webgpu-weather-control');
                this.api.log('ÃƒÂ°Ã…Â¸Ã¢â‚¬â„¢Ã‚Â¬ [WEATHER CONTROL] Unregistered GCCE commands', 'debug');
            }
        } catch (error) {
            this.api.log(`ÃƒÂ¢Ã‚ÂÃ…â€™ [WEATHER CONTROL] Error unregistering GCCE commands: ${error.message}`, 'error');
        }
        ...
        try {
            ...
            this.api.log(`ÃƒÂ¢Ã‚ÂÃ…â€™ [WEATHER CONTROL] Error persisting gamification state during shutdown: ${error.message}`, 'error');
        }
