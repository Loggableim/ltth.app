/**
 * Tests for EmojiRain gift ball behavior.
 */
...
describe('EmojiRain - Geschenk-Kugeln', () => {
  let EmojiRainPlugin;
  ...
  test('tier-based gift balls ignore the legacy min/max clamp and grow up to 5000px', () => {
    ...
    expect(api.emissions[0].event).toBe('emoji-rain:gift-balls');
  ...
  test('gift balls can be disabled without disabling normal gift emoji rain', () => {
    ...
    expect(api.emissions[0].event).toBe('emoji-rain:spawn');
  });
});
