/**
 * EmojiRain Plugin - Enhanced with GCCE Integration
 *
...
class EmojiRainPlugin {
  ...
  async init() {
    this.api.log('🌧️ [EmojiRain] Initializing Enhanced Emoji Rain Plugin...', 'info');
    ...
    if (!fs.existsSync(this.uploadDir)) {
      ...
      this.api.log('📁 [EmojiRain] Upload directory created', 'debug');
    } else {
      this.api.log('📁 [EmojiRain] Upload directory exists', 'debug');
    }

    this.api.log(`📂 [EmojiRain] Using persistent storage: ${this.uploadDir}`, 'info');

    // Initialize presets file if not exists
    if (!fs.existsSync(this.presetsPath)) {
      this.savePresets(this.getDefaultPresets());
      this.api.log('📋 [EmojiRain] Default presets initialized', 'info');
    }
    ...
    // Register routes
    this.api.log('🛣️ [EmojiRain] Registering routes...', 'debug');
    this.registerRoutes();

    // Register TikTok event handlers
    this.api.log('🎯 [EmojiRain] Registering TikTok event handlers...', 'debug');
    this.registerTikTokEventHandlers();

    // Register flow actions
    this.api.log('⚡ [EmojiRain] Registering flow actions...', 'debug');
    this.registerFlowActions();
    
    // Integrate with GCCE
    this.api.log('🎮 [EmojiRain] Integrating with GCCE...', 'debug');
    ...
    this.api.log('✅ [EmojiRain] Plugin initialized successfully with GCCE integration', 'info');
  }
  ...
  loadPresets() {
    try {
      ...
      this.api.log(`⚠️ [EmojiRain] Error loading presets: ${error.message}`, 'warn');
    }
  ...
  savePresets(presets) {
    try {
      ...
      this.api.log('💾 [EmojiRain] Presets saved', 'debug');
    } catch (error) {
      this.api.log(`❌ [EmojiRain] Error saving presets: ${error.message}`, 'error');
    }
  }
  ...
  emitOverlaySpawn(spawnData) {
    ...
    const eventName = isHeartBalloons
      ...
      : (isGiftBalls ? 'emoji-rain:gift-balls' : 'emoji-rain:spawn');
  ...
  triggerEmojiRain(params) {
    ...
    if (spawnData.emoji === '{{profilePicture}}') {
      if (spawnData.profilePictureUrl) {
        this.api.log(`🖼️ [EmojiRain] Profile picture spawn for ${spawnData.username}: ${spawnData.profilePictureUrl}`, 'debug');
      } else {
        this.api.log(`⚠️ [EmojiRain] Profile picture requested for ${spawnData.username} but no URL available - will use fallback`, 'warn');
      }
    }
  ...
  debugLog(message) {
    ...
    this.debugLogCount++;
    this.api.log(`🐛 [EmojiRain DEBUG] ${message}`, 'debug');
  ...
  /**
   * Copy the data of the formerly misnamed webgpu-emoji-rain plugin into the
   * canonical EmojiRain data namespace. Sources are intentionally retained.
   */
  async migrateOldData() {
    const previousPluginDataDir = typeof this.configPathManager.getPluginDataDir === 'function'
      ? this.configPathManager.getPluginDataDir('webgpu-emoji-rain')
      : null;
    const oldEmojiRainUploadDir = previousPluginDataDir ? path.join(previousPluginDataDir, 'uploads') : '';
    const previousMappingsPath = previousPluginDataDir ? path.join(previousPluginDataDir, 'users.json') : '';
    const previousPresetsPath = previousPluginDataDir ? path.join(previousPluginDataDir, 'presets.json') : '';
    ...
    const previousUserConfigsPath = path.join(this.configPathManager.getUserConfigsDir(), 'webgpu-emoji-rain', 'users.json');
    ...
    if (fs.existsSync(oldEmojiRainUploadDir)) {
      ...
      if (oldFiles.length > 0) {
        this.api.log(`📦 [EmojiRain] Migrating ${oldFiles.length} files from emoji-rain plugin...`, 'info');
        ...
        if (migrated) {
          this.api.log(`✅ [EmojiRain] Migrated uploads from emoji-rain`, 'info');
        }
      }
    }
    ...
    if (!fs.existsSync(this.userMappingsPath)) {
      ...
      // Priority 1: data from the previously misnamed plugin
      if (fs.existsSync(previousMappingsPath)) {
        this.api.log('📦 [EmojiRain] Migrating mappings from webgpu-emoji-rain data...', 'info');
        fs.copyFileSync(previousMappingsPath, this.userMappingsPath);
        migrated = true;
      }
      // Priority 2: persistent user_configs from the previous plugin identity
      else if (fs.existsSync(previousUserConfigsPath)) {
        this.api.log('📦 [EmojiRain] Migrating mappings from webgpu-emoji-rain user_configs...', 'info');
        fs.copyFileSync(previousUserConfigsPath, this.userMappingsPath);
        migrated = true;
      }
      // Priority 3: canonical persistent user_configs
      else if (fs.existsSync(this.userConfigMappingsPath)) {
        this.api.log('📦 [EmojiRain] Migrating user mappings from persistent user_configs...', 'info');
        fs.copyFileSync(this.userConfigMappingsPath, this.userMappingsPath);
        this.api.log(`✅ [EmojiRain] Migrated user mappings from user_configs`, 'info');
        migrated = true;
      }
      // Priority 4: old app user_configs
      else if (fs.existsSync(oldAppUserConfigsPath)) {
        this.api.log('📦 [EmojiRain] Migrating user mappings from old app user_configs...', 'info');
        ...
        this.api.log(`✅ [EmojiRain] Migrated user mappings from old app user_configs`, 'info');
        migrated = true;
      }
      // Priority 5: original emoji-rain data directory
      else if (fs.existsSync(oldDataPluginsDir)) {
        this.api.log('📦 [EmojiRain] Migrating user mappings from data directory...', 'info');
        fs.copyFileSync(oldDataPluginsDir, this.userMappingsPath);
        this.api.log(`✅ [EmojiRain] Migrated user mappings from emoji-rain plugin`, 'info');
        migrated = true;
      }
    } else {
      ...
      if (fs.existsSync(this.userConfigMappingsPath)) {
        ...
        if (userConfigStats.mtime > persistentStats.mtime) {
          this.api.log('📦 [EmojiRain] Updating user mappings from newer user_configs version...', 'info');
          fs.copyFileSync(this.userConfigMappingsPath, this.userMappingsPath);
          this.api.log(`✅ [EmojiRain] Updated user mappings from user_configs`, 'info');
          migrated = true;
        }
      }
    }

    if (!fs.existsSync(this.presetsPath) && fs.existsSync(previousPresetsPath)) {
      fs.mkdirSync(path.dirname(this.presetsPath), { recursive: true });
      fs.copyFileSync(previousPresetsPath, this.presetsPath);
      this.api.log('✅ [EmojiRain] Migrated presets from webgpu-emoji-rain', 'info');
      migrated = true;
    }

    if (migrated) {
      this.api.log('💡 [EmojiRain] Old files are kept for safety', 'info');
    }
  }
  ...
  registerRoutes() {
    ...
    this.api.registerRoute('get', '/emoji-rain/uploads/:filename', serveUploadedImage);
    this.api.registerRoute('get', '/uploads/emoji-rain/:filename', serveUploadedImage);
    ...
    // Test heart balloons
    this.api.registerRoute('post', '/api/emoji-rain/test-heart-balloons', (req, res) => {
      try {
    ...
    // Test gift ball
    this.api.registerRoute('post', '/api/emoji-rain/test-gift-ball', (req, res) => {
      try {
    ...
    this.api.registerRoute('post', '/api/emoji-rain/upload', (req, res) => {
      this.upload.single('image')(req, res, async (err) => {
        if (err) {
          this.api.log(`❌ [EmojiRain] Upload error: ${err.message}`, 'error');
        ...
        try {
          ...
          if (req.file.mimetype === 'image/svg+xml') {
            try {
              ...
              this.api.log(`🧼 [EmojiRain] SVG sanitized: ${req.file.filename}`, 'debug');
            } catch (sanitizeError) {
              this.api.log(`⚠️ [EmojiRain] SVG sanitization failed: ${sanitizeError.message}`, 'warn');
            }
          }
          ...
          this.api.log(`📤 [EmojiRain] Image uploaded by ${username}: ${req.file.filename} (${currentCount + 1}/${this.maxUploadsPerUser})`, 'info');
          ...
          this.api.log(`❌ [EmojiRain] Error processing upload: ${error.message}`, 'error');
          ...
          try {
            ...
            this.api.log(`⚠️ [EmojiRain] Cleanup error: ${cleanupError.message}`, 'warn');
          }
    ...
    this.api.registerRoute('get', '/api/emoji-rain/images', (req, res) => {
      try {
        ...
        this.api.log(`❌ [EmojiRain] Error listing images: ${error.message}`, 'error');
        res.status(500).json({ success: false, error: error.message });
      }
    });
    ...
    this.api.registerRoute('delete', '/api/emoji-rain/images/:filename', (req, res) => {
      try {
        ...
        this.api.log(`🗑️ [EmojiRain] Image deleted: ${filename}`, 'info');

        res.json({ success: true, message: 'Image deleted successfully' });
      } catch (error) {
        this.api.log(`❌ [EmojiRain] Error deleting image: ${error.message}`, 'error');
        res.status(500).json({ success: false, error: error.message });
      }
    });
    ...
    this.api.registerRoute('get', '/api/emoji-rain/user-mappings', (req, res) => {
      try {
        ...
        this.api.log(`❌ [EmojiRain] Error getting user mappings: ${error.message}`, 'error');
        res.status(500).json({ success: false, error: error.message });
      }
    });
    ...
    this.api.registerRoute('post', '/api/emoji-rain/user-mappings', (req, res) => {
      try {
        ...
        this.api.log(`💾 [EmojiRain] User mappings saved (${Object.keys(mappings).length} entries)`, 'info');
        ...
        this.api.log(`❌ [EmojiRain] Error updating user mappings: ${error.message}`, 'error');
        res.status(500).json({ success: false, error: error.message });
      }
    });

    // Export user mappings (bulk)
    this.api.registerRoute('get', '/api/emoji-rain/user-mappings/export', (req, res) => {
      try {
        let mappings = {};
        if (fs.existsSync(this.userMappingsPath)) {
          mappings = JSON.parse(fs.readFileSync(this.userMappingsPath, 'utf8'));
        ...
        this.api.log(`📤 [EmojiRain] User mappings exported`, 'info');
      } catch (error) {
        this.api.log(`❌ [EmojiRain] Error exporting user mappings: ${error.message}`, 'error');
        res.status(500).json({ success: false, error: error.message });
      }
    });

    // Import user mappings (bulk)
    this.api.registerRoute('post', '/api/emoji-rain/user-mappings/import', (req, res) => {
      try {
        ...
        // Notify overlays
        this.api.emit('emoji-rain:user-mappings-update', { mappings: finalMappings });
        
        this.api.log(`📥 [EmojiRain] User mappings imported (${Object.keys(finalMappings).length} entries)`, 'info');
        ...
        this.api.log(`❌ [EmojiRain] Error importing user mappings: ${error.message}`, 'error');
        res.status(500).json({ success: false, error: error.message });
      }
    });

    // Delete user mapping
    this.api.registerRoute('delete', '/api/emoji-rain/user-mappings/:username', (req, res) => {
      try {
        ...
        this.api.log(`🗑️ [EmojiRain] User mapping deleted: ${username}`, 'info');
        
        res.json({ success: true, message: `Mapping for ${username} deleted` });
      } catch (error) {
        this.api.log(`❌ [EmojiRain] Error deleting user mapping: ${error.message}`, 'error');
        res.status(500).json({ success: false, error: error.message });
      }
    });
    ...
    this.api.registerRoute('post', '/api/emoji-rain/trigger', (req, res) => {
      try {
        ...
        if (x !== undefined && x !== null && x !== '') {
          ...
          if (isNaN(parsedX)) {
            this.api.log(`⚠️ [EmojiRain] Invalid x coordinate received: ${x}, will use random`, 'warn');
            parsedX = undefined;
          }
        }
        ...
        if (y !== undefined && y !== null && y !== '') {
          ...
          if (isNaN(parsedY)) {
            this.api.log(`⚠️ [EmojiRain] Invalid y coordinate received: ${y}, will default to 0`, 'warn');
            parsedY = undefined;
          }
        }
        ...
        this.api.log(`❌ [EmojiRain] Error triggering emoji rain: ${error.message}`, 'error');
    ...
    this.api.registerRoute('post', '/api/emoji-rain/herzballons', (req, res) => {
      try {
    ...
    // Get all presets
    this.api.registerRoute('get', '/api/emoji-rain/presets', (req, res) => {
      try {
        const presets = this.loadPresets();
        res.json({ success: true, presets });
      } catch (error) {
        this.api.log(`❌ [EmojiRain] Error getting presets: ${error.message}`, 'error');
        res.status(500).json({ success: false, error: error.message });
      }
    });

    // Get specific preset
    this.api.registerRoute('get', '/api/emoji-rain/presets/:id', (req, res) => {
      try {
        const presets = this.loadPresets();
        ...
        this.api.log(`❌ [EmojiRain] Error getting preset: ${error.message}`, 'error');
        res.status(500).json({ success: false, error: error.message });
      }
    });

    // Create preset
    this.api.registerRoute('post', '/api/emoji-rain/presets', (req, res) => {
      try {
        ...
        this.api.log(`📋 [EmojiRain] Preset created: ${name}`, 'info');
        res.json({ success: true, preset: newPreset });
      } catch (error) {
        this.api.log(`❌ [EmojiRain] Error creating preset: ${error.message}`, 'error');
        res.status(500).json({ success: false, error: error.message });
      }
    });

    // Update preset
    this.api.registerRoute('put', '/api/emoji-rain/presets/:id', (req, res) => {
      try {
        const presets = this.loadPresets();
        ...
        this.api.log(`📋 [EmojiRain] Preset updated: ${req.params.id}`, 'info');
        res.json({ success: true, preset: presets[index] });
      } catch (error) {
        this.api.log(`❌ [EmojiRain] Error updating preset: ${error.message}`, 'error');
        res.status(500).json({ success: false, error: error.message });
      }
    });

    // Delete preset
    this.api.registerRoute('delete', '/api/emoji-rain/presets/:id', (req, res) => {
      try {
        const presets = this.loadPresets();
        ...
        this.api.log(`🗑️ [EmojiRain] Preset deleted: ${req.params.id}`, 'info');
        res.json({ success: true, message: 'Preset deleted' });
      } catch (error) {
        this.api.log(`❌ [EmojiRain] Error deleting preset: ${error.message}`, 'error');
        res.status(500).json({ success: false, error: error.message });
      }
    });

    // Trigger preset
    this.api.registerRoute('post', '/api/emoji-rain/presets/:id/trigger', (req, res) => {
      try {
        const config = this.api.getDatabase().getEmojiRainConfig();
        ...
        this.api.log(`❌ [EmojiRain] Error triggering preset: ${error.message}`, 'error');
        res.status(500).json({ success: false, error: error.message });
      }
    });
    ...
    // Pause overlay
    this.api.registerRoute('post', '/api/emoji-rain/overlay/pause', (req, res) => {
      try {
        this.overlayState.paused = true;
        this.api.emit('emoji-rain:pause', { paused: true });
        
        this.api.log('⏸️ [EmojiRain] Overlay paused', 'info');
        res.json({ success: true, message: 'Overlay paused' });
      } catch (error) {
        this.api.log(`❌ [EmojiRain] Error pausing overlay: ${error.message}`, 'error');
        res.status(500).json({ success: false, error: error.message });
      }
    });

    // Resume overlay
    this.api.registerRoute('post', '/api/emoji-rain/overlay/resume', (req, res) => {
      try {
        this.overlayState.paused = false;
        this.api.emit('emoji-rain:resume', { paused: false });
        ...
        if (this.spawnQueue.length > 0) {
          ...
          this.api.log(`▶️ [EmojiRain] Overlay resumed, processed ${queued.length} queued spawns`, 'info');
        } else {
          this.api.log('▶️ [EmojiRain] Overlay resumed', 'info');
        }
        
        res.json({ success: true, message: 'Overlay resumed' });
      } catch (error) {
        this.api.log(`❌ [EmojiRain] Error resuming overlay: ${error.message}`, 'error');
        res.status(500).json({ success: false, error: error.message });
      }
    });

    // Clear overlay
    this.api.registerRoute('post', '/api/emoji-rain/overlay/clear', (req, res) => {
      try {
        this.api.emit('emoji-rain:clear', {});
        this.spawnQueue = [];
        
        this.api.log('🧹 [EmojiRain] Overlay cleared', 'info');
        res.json({ success: true, message: 'Overlay cleared' });
      } catch (error) {
        this.api.log(`❌ [EmojiRain] Error clearing overlay: ${error.message}`, 'error');
        res.status(500).json({ success: false, error: error.message });
      }
    });

    // Set theme
    this.api.registerRoute('post', '/api/emoji-rain/overlay/theme', (req, res) => {
      try {
        ...
        this.api.emit('emoji-rain:theme', { theme });
        
        this.api.log(`🎨 [EmojiRain] Theme changed to: ${theme}`, 'info');
        res.json({ success: true, theme });
      } catch (error) {
        this.api.log(`❌ [EmojiRain] Error setting theme: ${error.message}`, 'error');
        res.status(500).json({ success: false, error: error.message });
      }
    });

    // Set opacity
    this.api.registerRoute('post', '/api/emoji-rain/overlay/opacity', (req, res) => {
      try {
        ...
        this.api.log(`❌ [EmojiRain] Error setting opacity: ${error.message}`, 'error');
        res.status(500).json({ success: false, error: error.message });
      }
    });

    // Set speed
    this.api.registerRoute('post', '/api/emoji-rain/overlay/speed', (req, res) => {
      try {
        ...
        this.api.log(`❌ [EmojiRain] Error setting speed: ${error.message}`, 'error');
        res.status(500).json({ success: false, error: error.message });
      }
    });

    // Set bounding box
    this.api.registerRoute('post', '/api/emoji-rain/overlay/bounding-box', (req, res) => {
      try {
        ...
        this.api.emit('emoji-rain:bounding-box', { boundingBox });
        
        this.api.log(`📐 [EmojiRain] Bounding box set: ${JSON.stringify(boundingBox)}`, 'info');
        res.json({ success: true, boundingBox });
      } catch (error) {
        this.api.log(`❌ [EmojiRain] Error setting bounding box: ${error.message}`, 'error');
        res.status(500).json({ success: false, error: error.message });
      }
    });

    // Get overlay state
    this.api.registerRoute('get', '/api/emoji-rain/overlay/state', (req, res) => {
      try {
        res.json({
          success: true,
    ...
    // Get metrics
    this.api.registerRoute('get', '/api/emoji-rain/metrics', (req, res) => {
      try {
        res.json({
          success: true,
        ...
        this.api.log(`❌ [EmojiRain] Error getting metrics: ${error.message}`, 'error');
        res.status(500).json({ success: false, error: error.message });
      }
    });

    // Reset metrics
    this.api.registerRoute('post', '/api/emoji-rain/metrics/reset', (req, res) => {
      try {
        this.metrics = {
          totalTriggers: 0,
          commandTriggers: 0,
        ...
        this.api.log('📊 [EmojiRain] Metrics reset', 'info');
        res.json({ success: true, message: 'Metrics reset' });
      } catch (error) {
        this.api.log(`❌ [EmojiRain] Error resetting metrics: ${error.message}`, 'error');
        res.status(500).json({ success: false, error: error.message });
      }
    });

    // Toggle debug mode
    this.api.registerRoute('post', '/api/emoji-rain/debug', (req, res) => {
      try {
        const { enabled } = req.body;
        ...
        this.api.log(`🐛 [EmojiRain] Debug mode ${this.debugMode ? 'enabled' : 'disabled'}`, 'info');
        res.json({ success: true, debugMode: this.debugMode });
      } catch (error) {
        this.api.log(`❌ [EmojiRain] Error toggling debug mode: ${error.message}`, 'error');
        res.status(500).json({ success: false, error: error.message });
      }
    });
    ...
    registerMiddleware('/plugins/emoji-rain/uploads', express.static(this.uploadDir));
    registerMiddleware('/uploads/emoji-rain', express.static(this.uploadDir));

    this.api.log('✅ [EmojiRain] All routes registered successfully', 'info');
  }
  ...
  registerTikTokEventHandlers() {
    ...
    this.api.log('✅ EmojiRain TikTok event handlers registered', 'info');
  }
  ...
  spawnEmojiRain(reason, data, count = null, emoji = null) {
    try {
      ...
      this.api.log(`❌ [EmojiRain] Error spawning emoji rain: ${error.message}`, 'error');
      this.metrics.lastError = error.message;
      this.metrics.lastErrorTime = new Date().toISOString();
    }
  }
  ...
  handleStickerEvent(data) {
    try {
      ...
      this.api.log(
        `🎭 [EmojiRain] Sticker ${isBurst ? 'BURST' : 'RAIN'}: ${count}x sticker by ${username} (Fan Level: ${teamMemberLevel})`,
        'info'
      );

      this.debugLog(`Sticker spawned: ${count}x for ${username} (level ${teamMemberLevel})${isBurst ? ' [BURST]' : ''}`);
    } catch (error) {
      this.api.log(`❌ [EmojiRain] Error handling sticker event: ${error.message}`, 'error');
      this.metrics.lastError = error.message;
      this.metrics.lastErrorTime = new Date().toISOString();
    }
  }
  ...
  registerFlowActions() {
    if (!this.api.registerFlowAction) {
      this.api.log('⚠️ [EmojiRain] Flow system not available, skipping flow action registration', 'warn');
      return;
    }

    // Register "Trigger EmojiRain" action
    this.api.registerFlowAction('emoji_rain_trigger', {
      name: 'Trigger EmojiRain',
      ...
      execute: async (params, eventData) => {
        try {
          ...
          this.api.log(`❌ [EmojiRain] Flow trigger error: ${error.message}`, 'error');
    ...
    this.api.registerFlowAction('emoji_rain_preset', {
      name: 'Trigger EmojiRain Preset',
      ...
      execute: async (params, eventData) => {
        try {
          ...
          this.api.log(`❌ [EmojiRain] Flow preset error: ${error.message}`, 'error');
          return { success: false, error: error.message };
        }
      }
    });
    ...
    this.api.registerFlowAction('emoji_rain_burst', {
      name: 'EmojiRain Burst',
      ...
      execute: async (params, eventData) => {
        try {
          ...
          this.api.log(`❌ [EmojiRain] Flow burst error: ${error.message}`, 'error');
          return { success: false, error: error.message };
        }
      }
    });
    ...
    this.api.registerFlowAction('emoji_rain_heart_balloons', {
      name: 'Herzballons',
    ...
    this.api.registerFlowAction('emoji_rain_clear', {
      name: 'Clear EmojiRain',
      ...
      execute: async (params, eventData) => {
        try {
          ...
          this.api.log(`❌ [EmojiRain] Flow clear error: ${error.message}`, 'error');
          return { success: false, error: error.message };
        }
      }
    });

    this.api.log('✅ [EmojiRain] Enhanced flow actions registered (5 actions)', 'info');
  }
  ...
  async destroy() {
    ...
    this.api.log('🌧️ [EmojiRain] Plugin destroyed', 'info');
  }
}
