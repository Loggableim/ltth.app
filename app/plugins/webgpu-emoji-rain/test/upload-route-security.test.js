...
describe('WebGPU Emoji Rain upload route security', () => {
  let tmpDir;
  let secretPath;
  let plugin;
  let api;
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'webgpu-emoji-rain-routes-'));

    jest.resetModules();
    const WebGPUEmojiRainPlugin = require('../main.js');
    api = new MockAPI(tmpDir);
    plugin = new WebGPUEmojiRainPlugin(api);
  ...
  test('does not serve files outside the upload directory', () => {
    const route = api.routes.find(
      entry => entry.method === 'get' && entry.routePath === '/webgpu-emoji-rain/uploads/:filename'
    );
  ...
  test('does not delete files outside the upload directory', () => {
    const route = api.routes.find(
      entry => entry.method === 'delete' && entry.routePath === '/api/webgpu-emoji-rain/images/:filename'
    );
    const res = createResponse();
