...
class AnimazingPalPlugin {
  ...
  shouldAcceptHostSpeech(text, metadata = {}) {
    ...
    if (features.isViewerAddressed) {
      return reject('viewer_addressed', 0, {
        ...features,
        isDirectRequest: false,
        isAddressedByName: false
      });
    }
    if (features.isGreeting && !features.isDirectRequest && !features.isAddressedByName) {
    ...
    if (!features.isDirectRequest && !features.isAddressedByName && !features.isQuestion) {
...
function findViewerAddressedByGreeting(text, names = []) {
  ...
  const directAddressPattern = new RegExp(`(?:^|[^\\p{L}\\p{N}_])(?:${namesPattern})(?:\\s*[,;:]|\\s+(?:antworte|antworten|reagier|reagiere|sag|erzaehl|erzÃ¤hl|kommentier|kommentiere|sprich|rede|gehts|geht|bitte|please|kannst|kann|du\\s+|du\\s+als|kann\\s+du)\\b)`, 'iu');
