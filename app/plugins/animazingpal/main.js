...
function tokenizeHostSpeechTextForEcho(text, maximum = 60) {

  const normalized = cleanHostSpeechText(text).toLocaleLowerCase();
...
function calculateHostSpeechEchoSimilarity(leftText, rightText) {

  const leftTokens = tokenizeHostSpeechTextForEcho(leftText, 2000);

  const rightTokens = tokenizeHostSpeechTextForEcho(rightText, 2000);
  ...
  return Math.min(1, Math.max(jaccard, coverage));
...
function splitSidekickAliases(raw) {
  return String(raw || '')
    .split(/[\n;,|]/)
    .map(item => cleanHostSpeechText(item, 80))
...
function findAddressedSidekickName(text, names = []) {
  ...
  for (const name of names) {
    const aliases = splitSidekickAliases(name);
    const aliasList = aliases.length ? aliases : [cleanHostSpeechText(name, 80)];
    for (const alias of aliasList) {
      const normalizedName = alias.toLocaleLowerCase();
...
function buildHostSpeechFeatures(text, response = {}, liveHost = {}) {
  ...
  const addressedViewer = findViewerAddressedByGreeting(clean, getLiveHostSidekickNames(liveHost));
  return {
    ...
    matchedSidekickName,
    isViewerAddressed: !!addressedViewer,
    addressedViewerName: addressedViewer || null,
...
function computeHostSpeechScore(features) {
  ...
  if (features.isGreeting && !features.isQuestion && !features.isDirectRequest) score -= 0.2;
  if (features.isViewerAddressed) score -= 0.4;
...
class AnimazingPalPlugin {
  ...
  constructor(api) {
    ...
    this.liveHostSpeechPlayback = {
      pendingCount: 0,
      recordsByKey: new Map(),
      keyBySpeechId: new Map(),
      resumeIdleMotion: false
    };
    this.liveHostSpeechPlaybackHandlers = null;
    ...
    return {
      ...
      brain: {
        ...
        autoRespond: {
          ...
          chat: true,               // Respond to chat messages
  ...
  normalizeConfig(config = {}) {
    ...
    const brainChatProbability = Number(normalized.brain.chatResponseProbability);
    const brainMaxResponses = Number(normalized.brain.maxResponsesPerMinute);
    const legacyChatConfig = (
      (Number.isFinite(brainChatProbability) && brainChatProbability <= 0.1)
      || (Number.isFinite(brainMaxResponses) && brainMaxResponses <= 4)
    );
    if (legacyChatConfig) {
      normalized.brain.autoRespond.chat = true;
    }
    if (Number.isFinite(brainChatProbability) && brainChatProbability <= 0.1) {
      normalized.brain.chatResponseProbability = 0.45;
    }
    if (Number.isFinite(brainMaxResponses) && brainMaxResponses <= 4) {
      normalized.brain.maxResponsesPerMinute = 18;
    }
    if (normalized.brain.liveHost?.response) {
      if (Number(normalized.brain.liveHost.response.chatProbability) <= 0.1) {
        normalized.brain.liveHost.response.chatProbability = 0.45;
      }
      if (Number(normalized.brain.liveHost.response.maxResponsesPerMinute) <= 4) {
        normalized.brain.liveHost.response.maxResponsesPerMinute = 18;
      }
    }
  ...
  registerRoutes() {
    ...
    this.api.registerRoute('post', '/api/animazingpal/config', async (req, res) => {
      try {
        ...
        // Preserve secrets that getSafeConfig() strips out
        if (newConfig?.brain && !newConfig.brain.openaiApiKey && this.config?.brain?.openaiApiKey) {
          mergedConfig.brain.openaiApiKey = this.config.brain.openaiApiKey;
        }
        if (newConfig?.brain?.liveHost?.providers) {
          for (const provider of ['openai', 'gemini', 'openrouter', 'ollama']) {
            const incoming = newConfig.brain.liveHost.providers[provider];
            if (incoming && (!incoming.apiKey || incoming.apiKey === '***configured***') && this.config?.brain?.liveHost?.providers?.[provider]?.apiKey) {
              mergedConfig.brain.liveHost.providers[provider].apiKey = this.config.brain.liveHost.providers[provider].apiKey;
            }
          }
        }
        if (newConfig?.viewerbase?.externalSync && !newConfig.viewerbase.externalSync.authToken && this.config?.viewerbase?.externalSync?.authToken) {
          mergedConfig.viewerbase.externalSync.authToken = this.config.viewerbase.externalSync.authToken;
        }
        if (newConfig?.platform?.profiles) {
          for (const [key, profile] of Object.entries(newConfig.platform.profiles)) {
            if (profile && profile.authToken === '' && this.config?.platform?.profiles?.[key]?.authToken) {
              mergedConfig.platform.profiles[key].authToken = this.config.platform.profiles[key].authToken;
            }
          }
        }
  ...
  resolveLiveHostSpeechLanguage(options = {}, liveHost = normalizeLiveHostConfig(this.config?.brain?.liveHost || {}, this.config?.brain || {}), fallback = 'de') {
    const configuredLanguage = normalizeLanguageCode(liveHost?.response?.language, fallback);
    const explicitLanguage = options?.language || options?.locale || options?.localeCode || options?.detectedLanguage || options?.metadata?.language;
    if (configuredLanguage === 'auto') {
      return normalizeLanguageCode(
        explicitLanguage || options?.asrLanguage || liveHost?.asr?.language || fallback,
        fallback
      );
    }
    return normalizeLanguageCode(explicitLanguage || configuredLanguage, fallback);
  }
  ...
  async speakHostResponse(message, options = {}) {
    ...
    const shouldResumeIdleMotion = Boolean(
      this.config?.enabled
      && liveHost.enabled
    ...
    const request = {
      ...
      language: this.resolveLiveHostSpeechLanguage(options, liveHost, 'de'),
    ...
    try {
      ...
      if (result?.success !== false && !result?.blocked) {
        playbackKey = this._trackLiveHostSpeechPlayback(result, request, shouldResumeIdleMotion);
      }

      try {
        if (result?.success !== false && !result?.blocked) {
          this._rememberLiveHostSpeech('assistant', request.text, {
      ...
      } catch (postTrackError) {
        if (playbackKey) {
          this._completeLiveHostSpeechPlayback(playbackKey, 'post-track-error');
        }
        throw postTrackError;
      }
  ...
  _ensureLiveHostSpeechPlaybackState() {
    if (!this.liveHostSpeechPlayback) {
      this.liveHostSpeechPlayback = {
        pendingCount: 0,
        recordsByKey: new Map(),
        keyBySpeechId: new Map(),
    ...
    return this.liveHostSpeechPlayback;
  }

  _isLiveHostSpeechPlaybackSource(source = '') {
    return String(source || '').toLowerCase().startsWith('animazingpal');
  }

  _registerLiveHostSpeechPlaybackBridge() {
    if (this.liveHostSpeechPlaybackHandlers || !this.api?.pluginLoader?.on) {
      return;
    }

    const startHandler = (payload = {}) => {
      if (!this._isLiveHostSpeechPlaybackSource(payload.source)) {
        return;
      }

      const speechId = payload.id;
      if (!speechId || !this.liveHostSpeechPlayback?.keyBySpeechId?.has(speechId)) {
        return;
      }

      const recordKey = this.liveHostSpeechPlayback.keyBySpeechId.get(speechId);
      const durationMs = Number(payload.duration);
      if (Number.isFinite(durationMs) && durationMs > 0) {
        this._scheduleLiveHostSpeechPlaybackTimeout(recordKey, Math.max(2500, durationMs + 1000));
      }
    };
    ...
    const endHandler = (payload = {}) => {
      ...
      const recordKey = this.liveHostSpeechPlayback.keyBySpeechId.get(speechId);
      this._completeLiveHostSpeechPlayback(recordKey, 'tts-ended');
    };

    this.api.pluginLoader.on('tts:playback:started', startHandler);
    this.api.pluginLoader.on('tts:playback:ended', endHandler);
    this.liveHostSpeechPlaybackHandlers = { startHandler, endHandler };
  }

  _scheduleLiveHostSpeechPlaybackTimeout(recordKey, delayMs) {
    const state = this._ensureLiveHostSpeechPlaybackState();
    const record = state.recordsByKey.get(recordKey);
    if (!record) {
      return;
    }
    ...
    record.timer = setTimeout(() => {
      this._completeLiveHostSpeechPlayback(recordKey, 'timeout');
    }, safeDelay);
  }

  _estimateLiveHostSpeechDurationMs(text, speed = 1) {
    ...
    const effectiveSpeed = Math.max(0.5, Number(speed) || 1);
    const wordsPerSecond = Math.max(1.6, 2.3 * effectiveSpeed);
    const estimateMs = Math.round((words / wordsPerSecond) * 1000) + 1200;
    return Math.max(3500, estimateMs);
  }

  _trackLiveHostSpeechPlayback(result = {}, request = {}, shouldResumeIdleMotion = false) {
    const state = this._ensureLiveHostSpeechPlaybackState();
    const recordKey = result?.id || `speech-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;

    state.recordsByKey.set(recordKey, {
      speechId: result?.id || null,
      timer: null
    });

    if (result?.id) {
      state.keyBySpeechId.set(result.id, recordKey);
    }

    state.pendingCount += 1;
    if (shouldResumeIdleMotion) {
      state.resumeIdleMotion = true;
    }

    if (state.pendingCount === 1 && !this.speechState?.isSpeaking?.()) {
      this.speechState?.markStarted?.();
    }

    this._scheduleLiveHostSpeechPlaybackTimeout(
      recordKey,
      Math.max(15000, this._estimateLiveHostSpeechDurationMs(request.text, request.speed) * 2)
    );

    return recordKey;
  }

  _completeLiveHostSpeechPlayback(recordKey, reason = 'ended') {
    const state = this.liveHostSpeechPlayback;
    if (!state || !recordKey || !state.recordsByKey.has(recordKey)) {
      return false;
    }
    ...
    if (record?.speechId) {
      state.keyBySpeechId.delete(record.speechId);
    }

    state.recordsByKey.delete(recordKey);
    ...
    if (state.pendingCount === 0) {
      ...
      this.liveHostLastSpeechAt = Date.now();
      if (state.resumeIdleMotion) {
        state.resumeIdleMotion = false;
        this.startLiveHostIdleMotion();
      }
    }
  ...
  recordLiveHostEventOutcome(eventType, result = {}) {
    ...
    const decisionReason = result.decision?.reason;
    const decisionReasons = result.decision?.respond === false
      ...
      ? (Array.isArray(result.decision.reasons) && result.decision.reasons.length > 0
        ...
        : (typeof decisionReason === 'string' && decisionReason.length > 0 ? [decisionReason] : ['low_signal']))
  ...
  startLiveHostSourceWatchdog() {
    ...
    const hasSourceUsername = Boolean(String(source.username || '').trim());

    const shouldWatch = Boolean(liveHost.enabled) && source.readOnly && source.autoConnect && hasSourceUsername;
    ...
    const interval = Math.max(5000, Number(source.watchdogIntervalMs) || 30000);
    ...
    this.liveHostSourceWatchdogTimer = setTimeout(() => {
      ...
      void (async () => {
        ...
        try {
          ...
          await this.runLiveHostSourceWatchdog();

        } catch (error) {

          if (this.api?.log) {

            this.api.log(`Live host source watchdog check failed: ${error?.message || error}`, 'warn');

          }

        } finally {

          const restart = normalizeLiveHostConfig(this.config?.brain?.liveHost || {}, this.config?.brain || {});

          if (Boolean(restart.enabled) && restart.source?.readOnly && restart.source?.autoConnect && restart.source?.username) {
  ...
  async runLiveHostSourceWatchdog() {
    ...
    const targetUsername = String(source.username || '').replace(/^@/, '').trim();

    this.liveHostSourceStatus.lastCheckedAt = new Date().toISOString();

    if (!source.readOnly || !source.autoConnect || !targetUsername || source.watchdogIntervalMs <= 0) {

      return { reconnected: false, reason: 'watchdog-disabled', status };

    }

    const tiktok = this.api?.tiktok || {};

    const isConnected = typeof tiktok.isActive === 'function'

      ? tiktok.isActive() === true

      : (typeof tiktok.isConnected === 'function' ? tiktok.isConnected() === true : Boolean(tiktok.connected));

    const currentUsername = String(tiktok.currentUsername || tiktok.username || '').replace(/^@/, '');

    const connectedToSource = Boolean(isConnected

      && targetUsername

      && (!currentUsername || currentUsername.toLowerCase() === targetUsername.toLowerCase()));

    if (!this.liveHostSourceReconnectInFlight) {

      const stale = source.reconnectOnEventStale && this.getLiveHostSourceEventStatus(liveHost).stale;

      if (isConnected && connectedToSource && !stale) {

        return { reconnected: false, reason: 'healthy', status: this.getLiveHostSourceStatus() };

      }

      if (typeof this.api.tiktok?.connect !== 'function') {

        this.liveHostSourceStatus.lastReconnectError = 'TikTok source connector unavailable';

        return { reconnected: false, reason: 'connect-unavailable', status: this.getLiveHostSourceStatus() };

      }

      this.liveHostSourceReconnectInFlight = true;

      this.liveHostSourceStatus.reconnectAttempts = (Number(this.liveHostSourceStatus.reconnectAttempts) || 0) + 1;

      this.liveHostSourceStatus.lastReconnectAt = new Date().toISOString();

      try {

        const result = await this.api.tiktok.connect(targetUsername);

        if (!result) {

          throw new Error('TikTok source reconnect returned false');
        ...
        return {
          ...
          reason: stale ? 'stale-events' : 'disconnected',

          status: this.getLiveHostSourceStatus()

        };

      } catch (error) {

        this.liveHostSourceStatus.lastReconnectError = String(error?.message || error);

        return { reconnected: false, reason: 'reconnect-failed', status: this.getLiveHostSourceStatus() };
    ...
    return { reconnected: false, reason: 'in-flight', status: this.getLiveHostSourceStatus() };
  ...
  async runLiveHostIdleMotionTick(now = Date.now()) {
    ...
    const speechRecoveryMs = Math.max(
      500,
      Number.isFinite(Number(idleMotion.cooldownAfterActionMs))
        ? Number(idleMotion.cooldownAfterActionMs)
        : 500
    );
    ...
    } else if (this.speechState?.isSpeaking()) {

      result.reason = 'speaking';

    } else if (now - (this.liveHostLastSpeechAt || 0) < speechRecoveryMs) {

      result.reason = 'speech-lock';
  ...
  evaluateLiveHostPreflight(options = {}) {
    ...
    const queueSize = Number(queueInfo?.size || 0);

    const queueMaxSize = Math.max(1, Number(queueInfo?.maxSize || 100));

    const queueWarnRatio = Math.min(1, Math.max(0, Number(liveHost.response?.queueWarnRatio ?? 0.8)));
    ...
    if (!queueInfo) {
      ...
      add('tts.queue', 'ok', 'TTS Queue', queueInfo ? `Queue: ${queueInfo.size || 0}/${queueInfo.maxSize || 100}.` : 'Queue-Status nicht verfÃƒÂ¼gbar, TTS-Plugin antwortet aber.');
        ...
        const animazeAudio = liveHost.animaze || {};
    const animazeOutputDeviceId = animazeAudio.audioOutputDeviceId || liveHost.audio.outputDeviceId;
    const animazeOutputDeviceLabel = animazeAudio.audioOutputDeviceLabel || liveHost.audio.outputDeviceLabel || animazeOutputDeviceId;
    const hasOutputDevice = Boolean(animazeOutputDeviceId);
    ...
    if (hasOutputDevice) {
      ...
      add(
        ...
        outputDeviceAvailable
          ...
          ? 'Das konfigurierte Animaze-Ausgabe-Device ist im Browser verfügbar.'
    ...
    add(
      ...
      playbackHasError
        ...
        : (playbackNeedsRoutingProof
          ...
          ? `Browser-TTS wurde noch nicht erfolgreich auf ${animazeOutputDeviceLabel} geroutet (${playback.status || 'noch kein Testlauf gemeldet'}).`
  ...
  decideLiveHostResponse(eventType, data = {}, event = {}, liveHost = buildLiveHostDefaults()) {
    ...
    if (eventType === 'chat') {
      ...
      // Base score for normal chat messages so they can be responded to
      score = 0.35;
      reasons.push('chat_base');
    ...
    const threshold = Number.isFinite(Number(response.minDecisionScore)) ? Number(response.minDecisionScore) : 0.35;
  ...
  async processLiveHostEvent(eventType, data = {}, options = {}) {
    ...
    const speechLanguage = this.resolveLiveHostSpeechLanguage({
      language: options.language || data.language,
      locale: data.locale,
      asrLanguage: liveHost?.asr?.language
    }, liveHost, 'de');
    ...
    if (event.templateEnabled && event.template) {
      ...
      if (message) {
        ...
        const speechResult = await this.speakHostResponse(message, {
    ...
    if (event.brainEnabled && this.brainEngine && eventType !== 'join') {
      ...
      const sidekickContext = {
        ...
        roleFlags: data.roleFlags || null,
        conversationState: data.conversationState || null,
        conversationHistory: Array.isArray(data.conversationHistory)
          ? data.conversationHistory
          : Array.isArray(data.conversationState?.recentTurns)
            ? data.conversationState.recentTurns
            : [],
        decision,
        eventType,
        source: data.source || options?.evaluation?.source || 'sidekick-viewer'
      };

      const method = {

        chat: () => this.brainEngine.processChat(username, data.comment || '', {
        ...
        gift: () => this.brainEngine.processGift(username, data.giftName || 'gift', coins || 1, {
        ...
        follow: () => this.brainEngine.processFollow(username, {
        ...
        share: () => this.brainEngine.processShare(username, {
        ...
        like: () => this.brainEngine.processLike(username, likes || 1, {
        ...
        subscribe: () => this.brainEngine.processSubscribe(username, {
    ...
    if (eventType === 'follow' && !responded && !speechFailureReason) {
      ...
      if (fallbackMessage) {
        const speechResult = await this.speakHostResponse(fallbackMessage, {
          eventType,
          username,
          userId: username,
          language: speechLanguage,
        ...
        recordSpeechAttempt(speechResult, fallbackMessage);
      }
    }
  ...
  async _handleAsrTranscribeRequest(req, res, readiness) {
    ...
    // Detect and reject repetitive hallucinations (e.g. 'no no no no no')
    const words = text.toLowerCase().split(/\s+/).filter(Boolean);
    if (words.length >= 2) {
      const uniqueWords = new Set(words);
      const repetitionRatio = uniqueWords.size / words.length;
      if (repetitionRatio <= 0.4 && words.length >= 2) {
        this.asrDiagnostics.counters.rejected += 1;
        this.api.log(`AnimazingPal ASR rejected repetitive hallucination: "${text.slice(0, 80)}" (${uniqueWords.size}/${words.length} unique words)`, 'warn');
        return res.json({
          success: true,
          transcript: this._redactAsrTranscript(transcript, text),
          accepted: false,
          delegated: false,
          reason: 'repetitive-hallucination',
          latencyMs,
    ...
    // Reject low-confidence transcripts if confidence is available
    const confidenceValue = Number(transcript?.confidence);
    if (Number.isFinite(confidenceValue) && confidenceValue < 0.2) {
      this.asrDiagnostics.counters.rejected += 1;
      this.api.log(`AnimazingPal ASR rejected low-confidence transcript: ${confidenceValue.toFixed(2)} "${text.slice(0, 80)}"`, 'warn');
      return res.json({
        success: true,
        transcript: this._redactAsrTranscript(transcript, text),
        accepted: false,
  ...
  _hasRecentLiveHostSpeechMatch(normalizedText, type, timestamp, windowMs) {
    ...
    const targetNormalized = normalizeHostSpeechText(normalizedText);
    if (!targetNormalized || !Array.isArray(this.liveHostHostSpeechHistory)) return false;

    const targetTokens = tokenizeHostSpeechTextForEcho(targetNormalized, 2000);
    ...
    return this.liveHostHostSpeechHistory.some(item => {
      ...
      const deltaMs = timestamp - Number(item.timestamp || 0);
      if (deltaMs < 0 || deltaMs > windowMs) return false;

      const itemText = cleanHostSpeechText(item.text || '');
      const itemNormalized = normalizeHostSpeechText(itemText);
      if (!itemNormalized) return false;

      const itemSource = String(item.metadata?.source || '');
      if (type === 'assistant' && itemSource && itemSource !== 'animazingpal-host-speech-output') {
        return false;
      }

      const itemTokens = tokenizeHostSpeechTextForEcho(itemNormalized, 2000);
      if (!itemTokens.length) return false;

      const similarity = calculateHostSpeechEchoSimilarity(itemNormalized, targetNormalized);
      const tokenLengthRatio = Math.min(itemTokens.length, targetTokens.length) / Math.max(itemTokens.length, targetTokens.length);

      return (
        item.normalizedText === targetNormalized
        || itemNormalized === targetNormalized
        || itemNormalized.includes(targetNormalized)
        || targetNormalized.includes(itemNormalized)
        || (
          similarity >= 0.72
          && tokenLengthRatio >= 0.55
          && itemTokens.length >= 3
  ...
  shouldAcceptHostSpeech(text, metadata = {}) {
    ...
    const conversationalFollowUp = isSidekickMode
      ...
      && !features.isGreeting
      && !features.isDirectRequest
      && !features.isAddressedByName
      && !features.isQuestion;
    ...
    if (features.isGreeting && !features.isDirectRequest && !features.isAddressedByName && !conversationalFollowUp) {
      return reject('greeting', 0, features);
    }

    const contextCooldownMs = Math.max(0, Number(response.hostContextCooldownMs) || 0);
    if (!conversationalFollowUp && !features.isDirectRequest && !features.isAddressedByName && contextCooldownMs > 0 && recentAcceptedSpeechAt && (timestamp - recentAcceptedSpeechAt) < contextCooldownMs) {
    ...
    if (!features.isDirectRequest && !features.isAddressedByName && !features.isQuestion && !conversationalFollowUp) {
      return reject('context_unclear', 0, features);
    }
    if (features.isLongForm && !conversationalFollowUp) return reject('context_unclear', 0, { ...features, isLongForm: true });


    const score = computeHostSpeechScore(features);
    const effectiveScore = conversationalFollowUp ? Math.min(1, score + 0.3) : score;

    const confidenceValue = Number.isFinite(confidence) ? confidence : null;

    if (confidenceValue !== null && confidenceValue < Number(response.hostMinConfidence) && !conversationalFollowUp) {
    ...
    if (effectiveScore < (Number.isFinite(minDecisionScore) ? minDecisionScore : 0.55)) {

      return reject('low_score', effectiveScore, { ...features, confidence: confidenceValue });

    }

    if (!features.isDirectRequest && !features.isAddressedByName && Math.random() > (Number(response.hostReplyProbability) * effectiveScore)) {
  ...
  async processSidekickHostSpeech(data = {}, evaluation = {}) {
    ...
    const speechLanguage = this.resolveLiveHostSpeechLanguage({
      language: evaluation.language || data.language,
      locale: data.locale,
      asrLanguage: liveHost?.asr?.language
    }, liveHost, 'de');
    ...
    const response = await this.brainEngine.processHostSpeech(username, message, {
      ...
      liveContext: {
        ...
        activeConversation: !!data.conversationState?.active,

        conversationSummary: data.conversationState?.summary || null
  ...
  async destroy() {
    ...
    if (this.liveHostSpeechPlaybackHandlers && this.api?.pluginLoader?.removeListener) {
      this.api.pluginLoader.removeListener('tts:playback:started', this.liveHostSpeechPlaybackHandlers.startHandler);
      this.api.pluginLoader.removeListener('tts:playback:ended', this.liveHostSpeechPlaybackHandlers.endHandler);
    }
    this.liveHostSpeechPlaybackHandlers = null;

    if (this.liveHostSpeechPlayback) {
      if (this.liveHostSpeechPlayback.recordsByKey) {
        for (const record of this.liveHostSpeechPlayback.recordsByKey.values()) {
          if (record?.timer) {
            clearTimeout(record.timer);
          }
        }
        this.liveHostSpeechPlayback.recordsByKey.clear();
      }
      if (this.liveHostSpeechPlayback.keyBySpeechId) {
        this.liveHostSpeechPlayback.keyBySpeechId.clear();
      }
      this.liveHostSpeechPlayback.pendingCount = 0;
      this.liveHostSpeechPlayback.resumeIdleMotion = false;
    }
...
function findViewerAddressedByGreeting(text, names = []) {
  const clean = cleanHostSpeechText(text).toLocaleLowerCase();
  if (!clean) return null;

  const normalizedNames = names
    .flatMap(name => splitSidekickAliases(name))
    .map(name => cleanHostSpeechText(name, 80).toLocaleLowerCase())
    .filter(name => name.length >= 2);

  if (!normalizedNames.length) return null;

  const escapedNames = normalizedNames.map(name => escapeHostSpeechRegex(name).replace(/\s+/g, '\\s+'));
  const namesPattern = escapedNames.join('|');
  const hasName = new RegExp(`(?:^|[^\\p{L}\\p{N}_])(?:${namesPattern})(?:[^\\p{L}\\p{N}_]|$)`, 'iu');
  if (!hasName.test(clean)) return null;

  const directGreeting = new RegExp(`(?:^|[^\\p{L}\\p{N}_])(hey|hi|hallo|servus|moin|guten tag|guten morgen|guten abend|hello|yo)\\s+(?:an\\s+)?(?:den|die|das\\s+)?(?:${namesPattern})\\b`, 'iu');
  if (directGreeting.test(clean)) {
